...
public class BisqCheckBoxSkin extends CheckBoxSkin {
    ...
    public BisqCheckBoxSkin(CheckBox control) {
        ...
        pressHandler = e -> {
            ...
            currentRipple = BisqRippler.pressAt(rippleOverlay, asRegionStub(), cx, cy,
    ...
    private void applyFillIntensity(double a) {
        if (box == null) return;
        // setStyle (inline) wins over CSS — without this, every pseudo-class change re-applies
        // CSS's `-fx-background-color: transparent` and competes with our Timeline, producing
        // a visible double-flash on uncheck.
        if (a <= 0) {
            box.setStyle("-fx-background-color: transparent;");
        } else {
            int alpha = (int) Math.round(Math.min(1, Math.max(0, a)) * 255);
            // rgba via #rrggbbaa lets us animate alpha smoothly.
            String hex = String.format("#%02x%02x%02x%02x", 0x0f, 0x9d, 0x58, alpha);
            box.setStyle("-fx-background-color: " + hex + ";");
        }
    }
    ...
    private Region asRegionStub() {
        Region r = new Region();
        r.setPrefSize(rippleOverlay.getWidth(), rippleOverlay.getHeight());
        r.resize(rippleOverlay.getWidth(), rippleOverlay.getHeight());
        return r;
    }
