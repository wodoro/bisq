...
public class BisqComboBoxSkin<T> extends ComboBoxListViewSkin<T> {
    ...
    public BisqComboBoxSkin(ComboBox<T> combo) {
        ...
        boolean floatCapable = combo instanceof LabelFloatable;
        showTopLabel = Bindings.createBooleanBinding(() -> {
            if (!floatCapable) return false;
            return ((LabelFloatable) combo).isLabelFloat();
        }, floatCapable ? ((LabelFloatable) combo).labelFloatProperty()
                        : combo.focusedProperty());
        topLabel.visibleProperty().bind(showTopLabel);

        getChildren().addAll(line, focusedLine, topLabel);

        combo.focusedProperty().addListener((o, ov, nv) -> { animateFocus(); updateFloatAnim(); });
        combo.valueProperty().addListener((o, ov, nv) -> updateFloatAnim());
        if (floatCapable) {
            ((LabelFloatable) combo).labelFloatProperty()
