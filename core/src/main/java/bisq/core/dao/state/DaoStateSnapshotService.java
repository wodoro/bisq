...
public class DaoStateSnapshotService implements DaoSetupService, DaoStateListener {
    ...
    private synchronized boolean applySnapshot(boolean fromInitialize,
        ...
        // when no snapshot proto exists, so `persistedDaoState` is never null here. Detect that
        // synthetic "no snapshot" state and treat it as a benign retry — same semantics as the
        // pre-async code path that returned early when nothing was persisted yet. Inconsistent
