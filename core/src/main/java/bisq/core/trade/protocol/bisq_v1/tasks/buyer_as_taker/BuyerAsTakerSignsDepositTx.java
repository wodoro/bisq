    ...
    protected void run() {
        try {
           ...
           /* log.debug("\n\n------------------------------------------------------------\n"
            ...
            // sequence/locktime that would make the deposit tx malleable. A malleated deposit
            // would invalidate the buyer-signed DPT and lock funds in the multisig output.
            TradeDataValidation.assertLockTimeIsZero(makersDepositTx);
            TradeDataValidation.assertInputSequencesAreFinal(makersDepositTx);
            TradeDataValidation.assertAllInputsAreP2WPKH(sellerInputs, walletService.getParams());
